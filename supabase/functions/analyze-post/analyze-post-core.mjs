...
export async function analyzePostWithOpenAI({
  ...
  const response = await fetchImpl(buildOpenAIResponsesUrl(baseUrl), {
    method: "POST",
    headers: {
      Authorization: `Bearer ${apiKey}`,
      "Content-Type": "application/json",
    },
    body: JSON.stringify(buildOpenAIAnalyzePostRequest({
      model,
      post,
      supportBoardSignal,
    })),
  ...
  return normalizeOpenAIAnalyzePostInsight(parseOpenAIResponseJson(data));
}
