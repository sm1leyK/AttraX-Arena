    ...
    function syncBalanceViews() {
        document.querySelectorAll('[data-user-balance]').forEach(el => {
            el.textContent = isLoggedIn ? `oute ${walletBalance}` : '访客模式';
        });
    }
        ...
        wrapper.innerHTML = `
            ...
            <div class="post-market-inline" data-post-id="${postId}">
                ...
                <div class="post-market-inline-actions">
                    <button class="post-market-inline-btn yes" data-bet-side="yes">站队 YES · 50 oute</button>
                    <button class="post-market-inline-btn no" data-bet-side="no">站队 NO · 50 oute</button>
                </div>
    ...
    function syncMarketElement(root, state) {
        ...
        if (balanceChip) {
            balanceChip.textContent = `oute ${walletBalance}`;
        }
        if (status && !status.dataset.locked) {
            status.textContent = '当前支持度已更新，可继续使用 oute 投分。';
        }
    }
    ...
    async function submitPostBet({ postId, side, stakeAmount }) {
        ...
        return {
            ...
            message: `已使用 ${stakeAmount} oute 站队 ${side.toUpperCase()}`
        };
    }
